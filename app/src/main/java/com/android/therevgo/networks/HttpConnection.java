...
public class HttpConnection {
    ...
    private static final int timeoutConnection = 4000;
    ...
    public static void RequestPost(final String url, final ArrayList<NameValuePair> valuePairs,
                                   final ResponseListener responseListner) {
         Runnable runnable = new Runnable() {
             ...
             public void run() {
                 try
                 {
                     ...
                     responseListner.onResponse(sl.getStatusCode() , object);

                 } catch (JSONException | ClientProtocolException e) {
                     e.printStackTrace();
                     responseListner.onError(e.toString());
                 } catch (ConnectTimeoutException e){
                     e.printStackTrace();
                     responseListner.onError("Please Check your internet connection.");
                 } catch (IOException e) {
                     e.printStackTrace();
                     responseListner.onError(e.toString());
                 }
    ...
    public static void RequestGet(final String URL, final ResponseListener responseListner){
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    if(responseCode == HttpURLConnection.HTTP_OK) {
                        ...
                        JSONObject object = new JSONObject(responseFromServer);
                        Log.i(RESPONSE,object.toString());
                        responseListner.onResponse(responseCode,object);
                    } else if (responseCode == HttpURLConnection.HTTP_NOT_FOUND) {
                        Log.i(RESPONSE,URL+" page Not Found");
                        responseListner.onError("Unable to connect server");
                    }
...
/*
    try
    {
        ...
        responseListner.onResponse(statusLine.getStatusCode() , new JSONObject(ResponseFromServer));
